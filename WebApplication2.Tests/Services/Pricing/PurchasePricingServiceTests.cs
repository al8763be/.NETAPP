...
public class PurchasePricingServiceTests
{
    ...
    [Fact]
    public void Calculate_AddsAboveCostProvisionBonus_WhenFinalPriceExceedsTotalCost()
    {
        var request = new PurchasePricingRequest
        {
            CustomerAge = 68,
        ...
        Assert.Equal(1600m, result.PriceGapAboveCost);
        Assert.Equal(320m, result.AboveCostProvisionBonus);
        Assert.Equal(2820m, result.TotalProvision);
    }
